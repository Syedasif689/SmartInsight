...
def should_check_duplicates() -> bool:
    ...
    max_bytes = (
        current_app.config.get("DUPLICATE_CHECK_MAX_MB", 5)
        * 1024
        * 1024
    )
...
def write_hash_file(file_path: Path, file_hash: str) -> None:
    ...
    try:
        hash_file_path(file_path).write_text(
            file_hash,
            encoding="utf-8",
        )
...
def get_uploaded_files():
    ...
    uploads.sort(
        key=lambda item: item["uploaded_at_timestamp"],
        reverse=True,
    )
